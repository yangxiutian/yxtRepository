...
//    public static final int DEFAULT_HEIGHT = 128;
    ...
    public static final int DEFAULT_FPS = 80;//原始值20
    public static final int DEFAULT_SPECTRUM_ANALYSER_FFT_SAMPLE_SIZE = 4096;//原始值512
    public static final int DEFAULT_SPECTRUM_ANALYSER_BAND_COUNT = 20;//柱状条数-原始值19
    ...
    private int peakBorder = 8;//自定义值，默认1
    ...
    public AudioChart(int width, int height) {
        DEFAULT_WIDTH = width;
...
//        }
        ...
        if (dsp == null) {
            dsp = new KJDigitalSignalProcessingAudioDataConsumer(DEFAULT_SPECTRUM_ANALYSER_FFT_SAMPLE_SIZE, DEFAULT_FPS);
            dsp.add(this);
        } else {
            dsp.remove(this);
            dsp = new KJDigitalSignalProcessingAudioDataConsumer(DEFAULT_SPECTRUM_ANALYSER_FFT_SAMPLE_SIZE, DEFAULT_FPS);
...
//        	System.out.println(pSample.length + "-->" + a);
        }
    }
    ...
    private void drawSpectrumAnalyserBar(Graphics pGraphics, int pX, int pY, int pWidth, int pHeight, int band) {
        ...
        if ((peakColor != null) && (peaksEnabled == true)) {
            ...
            pGraphics.fillRect(pX, pY - peaks[band], pWidth, peakBorder);
        }
    }
        ...
        addMouseListener(new MouseAdapter() {
            ...
            public void mousePressed(MouseEvent pEvent) {
                if (pEvent.getButton() == MouseEvent.BUTTON1) {
                    if (displayMode + 1 > 2) {//原始值>2,因为不要空白  所以改成>1
                        displayMode = 0;
                    } else {
                        displayMode++;
                    }
